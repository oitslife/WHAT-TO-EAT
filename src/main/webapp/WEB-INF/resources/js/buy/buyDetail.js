...
function addCart(){
	...
	console.log(buyInfo.item_id);
	console.log(buyInfo.user_id);
	console.log(buyInfo.url);
	const url = buyInfo.url;
	const jsonObj = {"item_id" : buyInfo.item_id,
					 "user_id" : buyInfo.user_id
				};
	...
	if(jsonObj.user_id.trim() == ""){
		...
		ppost(url, jsonObj, handler);		
	}
}

function ppost(url,jsonObj,handler){
	fetch(url, {
      method: 'POST',
      headers: {
...
function handler(response){
	if(response == "empty"){
		...
		window.alert("이미 장바구니에 저장되어 있습니다.");
	}
}
