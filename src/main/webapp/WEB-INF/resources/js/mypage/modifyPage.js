...
const $togglePw = document.getElementsByClassName('togglePw');

pwChangeBtn.addEventListener('click',(evt)=>{
evt.preventDefault();

const pw = null;
const url = "/portfolio/member/pw";
const jsonObj = {
	"member_id": modifyInfo.user_id,
	"birth": modifyInfo.user_birth,
	"telt": modifyInfo.user_tel
}

	console.log("비밀번호 변경 버튼이 클릭되었습니다.");
	if($beforePw.value.trim() == 0){
		console.log("비밀번호가 없습니다.");
		window.alert("현재 비밀번호를 입력해주세요");
		}else{
			chkPw(url, jsonObj, (json)=>{
				
				if($beforePw.value == json.result){
					console.log("비밀번호가 맞습니다.");
					$togglePw[0].style.display="";
					$togglePw[1].style.display="";
				}else{
...
function chkPw(url, jsonObj, controller){
	console.log("호출됨!");
    fetch(url, {
      method: 'POST',
      headers: {
...
$submitBtn.addEventListener('click', joinHandler);
