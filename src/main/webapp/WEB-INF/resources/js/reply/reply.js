...
const loginChk 		= (replyInfo.user_id === '') ? false : true;	
...
$comments.addEventListener('click',evt=>{
	let l_target = evt.target;
	  ...
	  if (l_target.tagName !== 'BUTTON') return; 
		...
		//buttonNameMap여기 창고에 등록해놓은 곡식 가져가려고 하는데요.
...
$paging.addEventListener("click",(e)=>{
  ...
  e.stopImmediatePropagation();
    ...
    let l_reqPage = e.target.closest('li').querySelector('a').getAttribute('href');	
	  g_reqPage = l_reqPage
    replyList(g_reqPage);
...
//댓글에 일단 들어오면 이건 생겨져 있다? 작성할 수 있는 란이 생성됨.
function addWriteHTML(){
  let $innerHTML = '';
	  $innerHTML += `<div class="profileImg">`;
	  $innerHTML += `	<img src="/portfolio/member/findProfileImg/${replyInfo.user_id}/" alt="프로파일">`;
	  $innerHTML += `</div>`;
	...
	$rcontent.addEventListener('keyup',evt=>{
			...
			if(evt.target.textContent.trim().length > 0){
				$writeBtn.style.backgroundColor = 'red';
			}else{
	...
	$writeBtn.addEventListener('click', evt=>{
		...
		"recipenum": replyInfo.recipenum,
	    "memberid" : replyInfo.user_id,
...
function replyList(reqPage =  1){
	...
	//목록가져오기 로직 추가  <===============================
	const url = `${replyInfo.url}/${replyInfo.recipenum}/${reqPage}`;  //요청URL
...
function rereplyFn(evt){
	...
	if( !$commentRewrite || !$commentRewrite.classList.contains('rewrite')) {
		...
		$rcontent.addEventListener('keyup',evt=>{
				//입력된 글자가 있으면
				if(evt.target.textContent.trim().length > 0){
					$btngrp.querySelector('.writeBtn').style.backgroundColor = 'red';
				}else{
					$btngrp.querySelector('.writeBtn').style.backgroundColor = 'blue';
				}
			});
